...
function getConnection(): ?mysqli
{
    $conn = new mysqli(
        ...
        null,
        '3ti_2024_25_cw1'
    ); //wywołanie konstruktora klasy mysqli
...
function saveStudentToDb(string $firstname, string $lastname, int $group_id): void
{
    $conn = getConnection();
    if ($conn == null) return;
    $sql = "INSERT INTO students(firstname,lastname,group_id) "
        . " VALUES ('{$firstname}','{$lastname}',{$group_id})";
    echo $sql;
    $conn->query($sql);
    $conn->close();
}
